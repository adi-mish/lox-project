...
## Test Matrix
- `pytest eloxir/tests -q` — **pass** (5 regression fixtures covering runtime edge cases).【3f3b9b†L1-L3】
- `python eloxir/tools/run_official_tests.py` — **fails** (257 passed / 8 failed / 265 total).【6a36cb†L1-L3】 The failing fixtures are enumerated below with their reproduction commands.
...
### 2. Loop body size limit is unenforced
- **Symptom:** `limit/loop_too_large.lox` runs to completion and prints hundreds of `nil`s instead of producing a compile-time error.【ceb236†L1-L6】
- **Cause:** `visitWhileStmt` simply lowers the loop without tracking emitted instruction counts, so the 65 535-instruction ceiling from the bytecode VM is never enforced.【F:eloxir/codegen/CodeGenVisitor.cpp†L1387-L1412】
...
- Regression coverage for the fixed issues lives in `eloxir/tests/regression/` and is exercised by `pytest` as part of the smoke test suite.【3f3b9b†L1-L3】
