...
CodeGenVisitor::CodeGenVisitor(llvm::Module& m)
    ...
    llvm::FunctionType* printFnTy = llvm::FunctionType::get(
        llvmValueTy(), {llvmValueTy()}, false);
    mod.getOrInsertFunction("elx_print", printFnTy);
    
    llvm::FunctionType* clockFnTy = llvm::FunctionType::get(
        llvmValueTy(), {}, false);
...
llvm::Value* CodeGenVisitor::tagOf(llvm::Value* v) {
    ...
    llvm::Value* mask = llvm::ConstantInt::get(llvm::Type::getInt64Ty(ctx), maskTag);
    return builder.CreateAnd(v, mask);
}

void CodeGenVisitor::visitBinaryExpr(Expr::Binary* e) {
    e->left->accept(this);
    llvm::Value* l = value;
    ...
    if (e->op.getType() == TokenType::PLUS) {
        ...
        phi->addIncoming(l, builder.GetInsertBlock()->getParent()->getEntryBlock()); // Default to left operand for error
...
void CodeGenVisitor::visitGroupingExpr(Expr::Grouping* e) {
    e->expression->accept(this);
    // value is already set by the nested expression
}

void CodeGenVisitor::visitLiteralExpr(Expr::Literal* e) {
    if (std::holds_alternative<double>(e->value)) {
        ...
        uint64_t nilValue = 0x7ffc000000000000ULL | (static_cast<uint64_t>(Tag::NIL) << 48);
        value = llvm::ConstantInt::get(llvmValueTy(), nilValue);
    } else if (std::holds_alternative<bool>(e->value)) {
        bool b = std::get<bool>(e->value);
        uint64_t boolValue = 0x7ffc000000000000ULL | (static_cast<uint64_t>(Tag::BOOL) << 48) | (b ? 1 : 0);
...
void CodeGenVisitor::visitUnaryExpr(Expr::Unary* e) {
    e->right->accept(this);
    // Simplified for demo
    // For a real implementation, this would handle operators like - and !
}

void CodeGenVisitor::visitVariableExpr(Expr::Variable* e) {
...
void CodeGenVisitor::visitAssignExpr(Expr::Assign* e) {
    // Simplified for demo
    e->value->accept(this);
    // In real implementation, this would store to the variable
}

void CodeGenVisitor::visitLogicalExpr(Expr::Logical* e) {
    // Simplified for demo
    e->left->accept(this);
    // In real implementation, this would implement short-circuit evaluation
}

void CodeGenVisitor::visitCallExpr(Expr::Call* e) {
    // Simplified for demo
...
void CodeGenVisitor::visitExpressionStmt(Stmt::Expression* s) {
    s->expression->accept(this);
    // Drop result (REPL wraps in print)
}

void CodeGenVisitor::visitPrintStmt(Stmt::Print* s) {
    s->expression->accept(this);
...
void CodeGenVisitor::visitVarStmt(Stmt::Var* s) {
    // Simplified for demo
    if (s->initializer) {
...
void CodeGenVisitor::visitBlockStmt(Stmt::Block* s) {
    // Simplified for demo
...
void CodeGenVisitor::visitIfStmt(Stmt::If* s) {
    // Simplified for demo
...
void CodeGenVisitor::visitWhileStmt(Stmt::While* s) {
    // Simplified for demo
    // In real implementation, this would create a loop
    s->body->accept(this);
}

void CodeGenVisitor::visitFunctionStmt(Stmt::Function* s) {
    // Simplified for demo
    // In real implementation, this would create a function
}

void CodeGenVisitor::visitReturnStmt(Stmt::Return* s) {
    // Simplified for demo
    if (s->value) {
        s->value->accept(this);
    } else {
...
void Print::codegen(CodeGenVisitor& codeGen) {
    codeGen.visitPrintStmt(this);
}

void Var::codegen(CodeGenVisitor& codeGen) {
    codeGen.visitVarStmt(this);
}

void Block::codegen(CodeGenVisitor& codeGen) {
    codeGen.visitBlockStmt(this);
}

void If::codegen(CodeGenVisitor& codeGen) {
    codeGen.visitIfStmt(this);
}

void While::codegen(CodeGenVisitor& codeGen) {
    codeGen.visitWhileStmt(this);
}
...
void Return::codegen(CodeGenVisitor& codeGen) {
    codeGen.visitReturnStmt(this);
}
}
